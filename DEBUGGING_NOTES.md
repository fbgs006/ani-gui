...
The Auto-Sync feature automatically updates the user's AniList progress (and marks the anime as `CURRENT`) when they finish an episode.
...
4. If the user's `time` is within 10 seconds of the AniSkip ED start time, it considers the episode "Finished" and syncs to AniList.
